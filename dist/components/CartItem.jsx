      ...
      <div className='flex items-start justify-between gap-4'>
        <div className='flex items-center space-x-4'>
          <div className='relative aspect-square h-16 w-16 min-w-fit overflow-hidden rounded'>
            {typeof image !== 'string' && image.url ? (<Image src={image.url} alt={product.name} fill className='absolute object-cover'/>) : (<div className='flex h-full items-center justify-center bg-secondary'>
          ...
          <div className='flex flex-col self-start'>
            ...
            <div className='mt-4 text-xs text-muted-foreground'>
              <button onClick={function () { return removeItem(product.id); }} className='flex items-center gap-0.5'>
                <X className='w-3 h-4'/>
                Remove
              </button>
            </div>
        ...
        <div className='flex flex-col space-y-1 font-medium'>
          <span className='ml-auto line-clamp-1 text-sm'>
            {/* Price or other product details can be added here */}
          </span>
        </div>
      </div>
    </div>);
