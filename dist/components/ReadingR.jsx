...
var ReadingRecommendations = function () {
    ...
    var visibleRecommendations = expanded ? recommendations : recommendations.slice(0, 3);
    return (<div className="sticky top-1">
      <section className="rounded-lg bg-gray-50 bg-opacity-40 px-6 py-8 shadow-md transition-all duration-300 ease-in-out">
        ...
        <ul className="space-y-4 mb-6">
          {visibleRecommendations.map(function (book, index) { return (<li key={index} className="flex items-center p-2 rounded-md transition-colors duration-200 hover:bg-gray-200">
        ...
        <Button className="w-full mt-4 bg-green-500 hover:bg-green-600 text-white transition-all duration-300 ease-in-out" onClick={function () { return setExpanded(!expanded); }}>
          {expanded ? (<>
