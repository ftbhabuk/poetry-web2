...
// @ts-ignore
var PerkCard = function (_a) {
    var perk = _a.perk;
    var isComingSoon = perk.name === 'Poetry Workshop';
    return (<div className="relative flex flex-col items-center p-8 bg-white bg-opacity-10 rounded-2xl shadow-lg transition-all duration-300 hover:transform hover:scale-105 hover:bg-opacity-20 group" style={{ border: '1px solid rgba(255, 255, 255, 0.2)' }}>
      <div className={"mb-6 p-4 rounded-full bg-gradient-to-br ".concat(perk.gradient, " shadow-lg")}>
      ...
      {isComingSoon && (<div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-70 flex items-center justify-center transition-all duration-300 rounded-2xl opacity-0 group-hover:opacity-100">
...
export default function Home() {
    return (<>
      <MaxWidthWrapper>
        ...
        <ProductReel query={{ sort: 'recent', limit: 4 }} href='/products?sort=recent' showSorting={false}/>
      </MaxWidthWrapper>
      ...
      <section className='border-t border-transparent py-28' style={{
        ...
        <MaxWidthWrapper>
          ...
          <div className='grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3'>
            {perks.map(function (perk) { return (<Link key={perk.name} href={perk.link || '#'}> {/* Updated to wrap PerkCard with Link */}
                <PerkCard perk={perk}/>
              </Link>); })}
          </div>
        </MaxWidthWrapper>
